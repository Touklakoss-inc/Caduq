...
namespace Caduq 
{
    ...
    private:
        ...
        std::vector<std::weak_ptr<Part>> m_PartList{ };
    ...
    public:
        ...
        const std::vector<std::weak_ptr<Part>>& GetPartList() { return m_PartList; };
        ...
        std::weak_ptr<Part>& GetPart(int index) { return m_PartList.at(index); };
    };
    ...
    void EntityManager::CreateEntity(const std::shared_ptr<T>& entity)
    {
        ...
        {
            m_PhyXManager->AddPhyXPartToList(entity->GetPhyXPart());
            m_PartList.push_back(entity);
        }
